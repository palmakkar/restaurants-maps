...
def find_predictor(user, restaurants, feature_fn):
    ...
    sum_of_squares = lambda x, y: sum([((x[elem] - mean(x)) * (y[elem] - mean(y))) for elem in range(0, len(x))])
    # BEGIN Question 7
    Sxx = sum_of_squares(xs, xs)
    Syy = sum_of_squares(ys, ys)
    Sxy = sum_of_squares(xs, ys)
    b, a, r_squared = Sxy/Sxx, mean(ys) - ((Sxy/Sxx) * mean(xs)), (Sxy*Sxy)/(Sxx * Syy)
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return find_predictor(user, reviewed, max(feature_fns, key = lambda feature: find_predictor(user, reviewed, feature)[1]))[0]
    # END Question 8
